...
const testscreen = () => {
  ...
  return (
    <View style={{backgroundColor:colors.background,flex:1,justifyContent:'flex-start',padding:spacing.md,gap:spacing.md}}> 
      <Text style={{fontFamily:fonts.headingHeavy,color:colors.text,fontSize:24}}>ADD EXERCISE</Text>
      ...
      renderItem={({ item }) => (
        <Card>
          <Text style={{ color: colors.text,fontFamily:fonts.bodyBold}}>{item.exerciseName}</Text>
          <Text style={{ color: colors.textMuted,fontFamily:fonts.body }}>
            {item.muscleGroup}
          </Text>
        </Card>
